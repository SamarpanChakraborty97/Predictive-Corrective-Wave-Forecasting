...
%RELATIVE PATHS%
%%%%%%%%%%%%%%%%
pathToSPHCode = 'C:/Users/schakr18/source/repos/Trial_WCSPH_cpu/Debug/';
%pathToSPHCode = '/~/scratch/SPH2D/';
%pathToSPHCode = '../';
pathToHere = mfilename('fullpath');
pathToDataFileStorage = 'dataFiles/';
nameOfInputFile       = 'SPHInputFile_Initialization.txt';
...
s.free.cSound      = 20 * max(ch,vmax);
s.free.nu          = 1 * 10^(-6);
s.free.delta       = 0.1; %kinematic viscosity
s.free.muAlpha     = 0.001; %-0.4
s.free.muBeta      = 1.0;  %muBeta -0.5
